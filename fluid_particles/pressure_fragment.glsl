...
void main() {
    ...
    float pRight = texture2D(fields_current, wrap(texCoord + vec2(Step.x, 0.0))).r;
    float pLeft = texture2D(fields_current, wrap(texCoord - vec2(Step.x, 0.0))).r;
    float pTop = texture2D(fields_current, wrap(texCoord + vec2(0.0, Step.y))).r;
    float pBottom = texture2D(fields_current, wrap(texCoord - vec2(0.0, Step.y))).r;
    
    float pCurrent = texture2D(fields_current, texCoord).r;
    
    // Average grid spacing
    float h = (Step.x + Step.y) / 2.0;
    float h_squared = h * h;
